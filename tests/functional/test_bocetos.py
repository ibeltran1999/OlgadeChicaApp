...
class BocetosFunctionalTestCase(unittest.TestCase):
    ...
    def test_registro_consulta_y_reinicio(self):
        respuesta = self.client.post(
            "/bocetos", json={"descripcion": "Estudio sin fecha"}
        )
        ...
        self.assertIn(
            "Estudio sin fecha",
            cliente.get("/bocetos/" + identificador).get_data(as_text=True),
        )
        self.assertIn(
            "Estudio sin fecha", cliente.get("/bocetos").get_data(as_text=True)
        )
    ...
    def test_archivo_fecha_tags_y_relacion_persisten(self):
        ...
        ).get_json()["identificador"]
        for i in range(3):
            self.client.post(
                "/tags", json={"identificador": str(i), "nombre": "Tag " + str(i)}
            )
        respuesta = self.client.post(
            "/bocetos",
            data={
                ...
                "fecha": "2021-01-01",
                "filmina_identificador": filmina,
                "tag_identificador": ["0", "1", "2"],
    ...
    def test_rechaza_asociaciones_invalidas(self):
        for extra in (
            ...
            {"tags": ["x", "x"]},
            {"tags": ["1", "2", "3", "4"]},
            {"filmina_identificador": "ausente"},
            {"fecha": "incorrecta"},
        ):
            with self.subTest(extra=extra):
                respuesta = self.client.post(
                    "/bocetos", json=dict(descripcion="No guardar", **extra)
                )
                self.assertEqual(respuesta.status_code, 400)
        self.assertNotIn(
            "No guardar", self.client.get("/bocetos").get_data(as_text=True)
        )

    def test_formulario_y_estado_vacio(self):
        self.assertEqual(self.client.get("/bocetos/nuevo").status_code, 200)
        self.assertIn(
            "Aún no hay bocetos", self.client.get("/bocetos").get_data(as_text=True)
        )

    def test_registros_simultaneos_reciben_numeros_distintos(self):
        app = create_app()

        def registrar(numero):
            with app.test_client() as cliente:
                respuesta = cliente.post(
                    "/bocetos", json={"descripcion": f"Boceto {numero}"}
                )
                self.assertEqual(respuesta.status_code, 201)
                return respuesta.get_json()["identificador"]

        with ThreadPoolExecutor(max_workers=4) as ejecutor:
            identificadores = list(ejecutor.map(registrar, range(8)))
        self.assertEqual(len(set(identificadores)), 8)
