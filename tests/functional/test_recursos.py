...
class RecursosFunctionalTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.filmina = self.client.post(
            "/filminas",
            json=dict(descripcion="Filmina", fecha="2020-01-01", procedencia="BLAA"),
        ).get_json()["identificador"]
        self.boceto = self.client.post(
            "/bocetos", json=dict(descripcion="Boceto")
        ).get_json()["identificador"]

    def test_registra_consulta_y_continua_numeracion_tras_reinicio(self):
        datos = dict(
            nombre="Obra conservada",
            tipo="Obra fisica",
            filmina_identificador=self.filmina,
        )
        ...
        self.assertIn(
            "Obra conservada",
            cliente.get("/recursos/" + identificador).get_data(as_text=True),
        )
        self.assertIn(
            "Obra conservada", cliente.get("/recursos").get_data(as_text=True)
        )
    ...
    def test_formulario_archivo_y_ambas_relaciones(self):
        respuesta = self.client.post(
            ...
            data=dict(
                ...
                filmina_identificador=self.filmina,
                boceto_identificador=self.boceto,
    ...
    def test_rechaza_asociaciones_y_tipo_invalidos(self):
        for extras in (
            ...
            {"boceto_identificador": "ausente"},
            {"filmina_identificador": self.filmina, "tipo": "Otro"},
        ):
            datos = dict(nombre="No guardar", tipo="Obra fisica")
            datos.update(extras)
            with self.subTest(extras=extras):
                self.assertEqual(
                    self.client.post("/recursos", json=datos).status_code, 400
                )
        self.assertNotIn(
            "No guardar", self.client.get("/recursos").get_data(as_text=True)
        )
    ...
    def test_formulario_y_navegacion_pendiente(self):
        ...
        self.assertIn(
            'href="/tags"', self.client.get("/tags/nueva").get_data(as_text=True)
        )
